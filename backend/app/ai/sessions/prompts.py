...
def compute_mcq_wrong_items(task_content: dict, user_response: dict) -> list[dict]:
    ...
    for item in task_content.get("items", []):
        ...
        if selected_idx != correct_idx:
            ...
            correct_label = (
                options[correct_idx] if correct_idx < len(options) else str(correct_idx)
            )
            wrong.append(
                {
                    "item_id": item_id,
...
def compute_listen_cloze_wrong_items(
...
def compute_error_spotting_wrong_items(
    task_content: dict, user_response: dict
) -> list[dict]:
    """Find missed and falsely selected word-level error-spotting items."""
    selected_ids = {
        str(token_id) for token_id in (user_response.get("selected_token_ids") or [])
    }
...
def build_task_gen_user_prompt(
    *,
    ...
    if spec.get("instructions_override"):
        parts.append(
            f"Author instructions (use verbatim): {spec['instructions_override']}"
        )
    ...
    if widget_key == "listen_and_respond" and archetype.archetype_id not in {
        "LISTEN_DICTATION",
        "LISTEN_RETELL",
        "LISTEN_SHADOW",
    }:
    ...
    if (
        archetype.archetype_id == "WRITE_ERROR_CORR"
        or archetype.ui_widget == "ErrorCorrection"
    ):
        parts.extend(
            [
