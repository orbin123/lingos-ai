...
def upgrade() -> None:
    ...
    op.create_table('feedback_memory_logs',
    ...
    op.create_index('ix_feedback_memory_session', 'feedback_memory_logs', ['session_id'], unique=False)
    op.create_index('ix_feedback_memory_user', 'feedback_memory_logs', ['user_id'], unique=False)

    # 2. Add mentor_note column to session_scorecards
    op.add_column('session_scorecards', sa.Column('mentor_note', sa.Text(), nullable=True))
