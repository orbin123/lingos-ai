"""Cycle 3 structural integrity lint — weeks 9-12, days 1-7 (28 days).

Mirror of ``test_cycle2_day_integrity`` for the B1 cycle authored in
``source_24w.py``. It guards the *structure* of every Cycle-3 day so the
human's manual live pass is feasible: the authored blueprints must flatten
into a runtime shape that agrees with the canonical archetype contract
registry. It does not exercise the LLM or DB.
"""
...
# Cycle 3 = weeks 9-12 × days 1-7. ``day_index`` is 0-based.
CYCLE3_CASES: list[tuple[int, int]] = [
    (week, day_index) for week in range(9, 13) for day_index in range(7)
]
CYCLE3_IDS: list[str] = [
    f"day_24_{week:02d}_{day_index + 1:02d}" for week, day_index in CYCLE3_CASES
]

# Every authored day ships in this fixed activity order (read, listen, write,
# speak) — same as Cycles 1 and 2.
EXPECTED_ACTIVITY_ORDER = ["read", "listen", "write", "speak"]

# Cycle 3 mirrors Cycle 2's archetypes day-for-day, so it covers the same
# canonical set of 34.
EXPECTED_CYCLE3_ARCHETYPE_COUNT = 34


@pytest.mark.parametrize("week,day_index", CYCLE3_CASES, ids=CYCLE3_IDS)
def test_cycle3_day_is_valid(week: int, day_index: int) -> None:
    ...
    assert day.cefr_level == "B1", f"{expected_id}: Cycle 3 must be B1"
    ...
    assert all(c["mandatory"] is True for c in day.activity_contracts), (
        f"{expected_id}: every Cycle-3 activity must be mandatory"
    )
    ...
    # C. Archetype ↔ registry alignment (widget equalities also verify any
    # eval/feedback override in source matches the canonical registry).
...
@pytest.mark.parametrize("week,day_index", CYCLE3_CASES, ids=CYCLE3_IDS)
def test_cycle3_persona_round_trip(week: int, day_index: int) -> None:
    day = file_source.get_day(week, day_index)
...
def test_cycle3_archetype_coverage() -> None:
    cycle3_archetypes = {
        archetype_id
        for week, day_index in CYCLE3_CASES
        for archetype_id in file_source.get_day(week, day_index).task_archetypes_used
    }

    assert len(cycle3_archetypes) == EXPECTED_CYCLE3_ARCHETYPE_COUNT, (
        f"expected {EXPECTED_CYCLE3_ARCHETYPE_COUNT} unique Cycle-3 archetypes, "
        f"found {len(cycle3_archetypes)}: {sorted(cycle3_archetypes)}"
    )

    for archetype_id in sorted(cycle3_archetypes):
