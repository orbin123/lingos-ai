"""Cycle 6 structural integrity lint — weeks 21-24, days 1-7 (28 days).

Mirror of ``test_cycle5_day_integrity`` for the C1 Mastery cycle authored in
``source_24w.py``. Structural layout mirrors weeks 17-20 (W-4).
"""
...
CYCLE6_CASES: list[tuple[int, int]] = [
    (week, day_index) for week in range(21, 25) for day_index in range(7)
]
CYCLE6_IDS: list[str] = [
    f"day_24_{week:02d}_{day_index + 1:02d}" for week, day_index in CYCLE6_CASES
]

EXPECTED_ACTIVITY_ORDER = ["read", "listen", "write", "speak"]
EXPECTED_CYCLE6_ARCHETYPE_COUNT = 34
STRUCTURAL_MIRROR_OFFSET = 4


@pytest.mark.parametrize("week,day_index", CYCLE6_CASES, ids=CYCLE6_IDS)
def test_cycle6_day_is_valid(week: int, day_index: int) -> None:
    ...
    assert all(c["mandatory"] is True for c in day.activity_contracts), (
        f"{expected_id}: every Cycle-6 activity must be mandatory"
    )
...
@pytest.mark.parametrize("week,day_index", CYCLE6_CASES, ids=CYCLE6_IDS)
def test_cycle6_persona_round_trip(week: int, day_index: int) -> None:
    day = file_source.get_day(week, day_index)
...
def test_cycle6_archetype_coverage() -> None:
    cycle6_archetypes = {
        archetype_id
        for week, day_index in CYCLE6_CASES
        for archetype_id in file_source.get_day(week, day_index).task_archetypes_used
    }

    assert len(cycle6_archetypes) == EXPECTED_CYCLE6_ARCHETYPE_COUNT, (
        f"expected {EXPECTED_CYCLE6_ARCHETYPE_COUNT} unique Cycle-6 archetypes, "
        f"found {len(cycle6_archetypes)}: {sorted(cycle6_archetypes)}"
    )
