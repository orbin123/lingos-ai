"""Cycle 5 structural integrity lint — weeks 17-20, days 1-7 (28 days).

Mirror of ``test_cycle4_day_integrity`` for the B2 Reasoning cycle in
``source_24w.py``.
"""
...
CYCLE5_CASES: list[tuple[int, int]] = [
    (week, day_index) for week in range(17, 21) for day_index in range(7)
]
CYCLE5_IDS: list[str] = [
    f"day_24_{week:02d}_{day_index + 1:02d}" for week, day_index in CYCLE5_CASES
]

EXPECTED_ACTIVITY_ORDER = ["read", "listen", "write", "speak"]
EXPECTED_CYCLE5_ARCHETYPE_COUNT = 34


@pytest.mark.parametrize("week,day_index", CYCLE5_CASES, ids=CYCLE5_IDS)
def test_cycle5_day_is_valid(week: int, day_index: int) -> None:
    expected_id = f"day_24_{week:02d}_{day_index + 1:02d}"

    day = file_source.get_day(week, day_index)
    assert day.day_id == expected_id
    assert day.cefr_level == "C1", f"{expected_id}: Cycle 5 must be C1"
    ...
    assert all(c["mandatory"] is True for c in day.activity_contracts), (
        f"{expected_id}: every Cycle-5 activity must be mandatory"
    )
...
@pytest.mark.parametrize("week,day_index", CYCLE5_CASES, ids=CYCLE5_IDS)
def test_cycle5_persona_round_trip(week: int, day_index: int) -> None:
    day = file_source.get_day(week, day_index)
...
def test_cycle5_archetype_coverage() -> None:
    cycle5_archetypes = {
        archetype_id
        for week, day_index in CYCLE5_CASES
        for archetype_id in file_source.get_day(week, day_index).task_archetypes_used
    }

    assert len(cycle5_archetypes) == EXPECTED_CYCLE5_ARCHETYPE_COUNT, (
        f"expected {EXPECTED_CYCLE5_ARCHETYPE_COUNT} unique Cycle-5 archetypes, "
        f"found {len(cycle5_archetypes)}: {sorted(cycle5_archetypes)}"
    )
...
@pytest.mark.parametrize("week,day_index", CYCLE5_CASES, ids=CYCLE5_IDS)
def test_cycle5_mirrors_cycle4_archetypes(week: int, day_index: int) -> None:
    """Day-for-day archetype layout must match week W-4 (cycle 4)."""
