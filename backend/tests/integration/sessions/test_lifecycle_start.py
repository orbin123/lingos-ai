...
class TestBackgroundTaskTracking:
    """Guards the fix for the lazy-prefill GC footgun.

    The chat layer builds a *transient* SessionService per request and drops it
    the moment ``start_session`` returns. The event loop only weak-refs Tasks,
    so a fire-and-forget prefill task held solely on that instance is collectable
    mid-generation. ``_track_background`` keeps a process-lived strong ref in
    ``_BACKGROUND_TASKS`` until the task finishes, then releases it.
    """

    @pytest.mark.asyncio
    async def test_track_background_holds_process_lived_ref(self, db_session):
        from app.modules.sessions import service as service_module
        ...
        try:
            service._track_background(task)

            # While in flight, a strong ref exists at module scope — independent
            # of the (transient) service — so the loop's weak ref can't GC it.
            assert task in service_module._BACKGROUND_TASKS
            assert task in service._pending_rag_tasks

            release.set()
            await task

            # The done-callback releases both refs.
            assert task not in service_module._BACKGROUND_TASKS
            assert task not in service._pending_rag_tasks
        finally:
            release.set()
            if not task.done():
                await task
