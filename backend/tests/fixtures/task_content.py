"""Canonical valid task-content builders, one per archetype.

Extracted from `test_task_content_validation.py` so multiple test modules
(content-validation, contract-validation-gate, the LLM session-agent tests)
share ONE source of representative payloads instead of importing across test
files (which breaks when files move). Pure data builders — no DB, no fixtures.
"""
