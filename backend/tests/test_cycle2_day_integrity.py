"""Cycle 2 structural integrity lint — weeks 5-8, days 1-7 (28 days).

Mirror of ``test_cycle1_day_integrity`` for the A2 cycle authored in
``source_24w.py``. It guards the *structure* of every Cycle-2 day so the
...
# Cycle 2 = weeks 5-8 × days 1-7. ``day_index`` is 0-based.
CYCLE2_CASES: list[tuple[int, int]] = [
    (week, day_index) for week in range(5, 9) for day_index in range(7)
]
CYCLE2_IDS: list[str] = [
    f"day_24_{week:02d}_{day_index + 1:02d}" for week, day_index in CYCLE2_CASES
]

# Every authored day ships in this fixed activity order (read, listen, write,
# speak) — same as Cycle 1.
EXPECTED_ACTIVITY_ORDER = ["read", "listen", "write", "speak"]

# Cycle 2 mirrors Cycle 1's archetypes day-for-day, so it covers the same
# canonical set of 34.
EXPECTED_CYCLE2_ARCHETYPE_COUNT = 34


@pytest.mark.parametrize("week,day_index", CYCLE2_CASES, ids=CYCLE2_IDS)
def test_cycle2_day_is_valid(week: int, day_index: int) -> None:
    ...
    assert day.cefr_level == "A2", f"{expected_id}: Cycle 2 must be A2"
    ...
    assert all(c["mandatory"] is True for c in day.activity_contracts), (
        f"{expected_id}: every Cycle-2 activity must be mandatory"
    )
...
@pytest.mark.parametrize("week,day_index", CYCLE2_CASES, ids=CYCLE2_IDS)
def test_cycle2_persona_round_trip(week: int, day_index: int) -> None:
    day = file_source.get_day(week, day_index)
...
def test_cycle2_archetype_coverage() -> None:
    cycle2_archetypes = {
        archetype_id
        for week, day_index in CYCLE2_CASES
        for archetype_id in file_source.get_day(week, day_index).task_archetypes_used
    }

    assert len(cycle2_archetypes) == EXPECTED_CYCLE2_ARCHETYPE_COUNT, (
        f"expected {EXPECTED_CYCLE2_ARCHETYPE_COUNT} unique Cycle-2 archetypes, "
        f"found {len(cycle2_archetypes)}: {sorted(cycle2_archetypes)}"
    )
