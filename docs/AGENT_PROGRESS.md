...
## Session 8 — Gap G7: Security / hardening close-out (Phase D, final gap)

**Status:** **code complete; live proof + founder console work are founder-gated.**
The PR ships HSTS on both hosts, re-confirms the prod guard (test-proven), and
resolves the `backup*.sql` history question with a documented decision + procedure.
The remaining G7 items — confirming HSTS headers on live `api`/`www` after deploy,
rotating provider keys off dev-seeded values, and executing (or formally accepting)
the `backup*.sql` history decision — are founder runbook steps. G7 is **not** marked
done until those are recorded.
**Branch:** `feat/g7-security-closeout`
**Date:** 2026-06-19

### Why this gap
G1–G6 and G8 are merged (`#101`–`#115`). G7 is the **only** remaining gap in the plan
and the last of Phase D; Session 7 explicitly recommended it next. Closing it ticks the
final Appendix-C boxes → launch.

### Critical finding (shaped the gap)
Two of G7's five items needed **no build**: the `config.py` prod guard
(`_guard_production`, [config.py:276](../backend/app/core/config.py)) already refuses to
boot on every unsafe combination and is fully covered by
`tests/unit/platform/test_config_prod_guard.py` (8 unsafe combos rejected) — "re-confirm"
is verification, not code. The real code holes were **HSTS** (backend `main.py` added
CORS + 3 logging/rate-limit middlewares but **no security headers**; `next.config.ts` had
no `headers()` and Vercel sends no HSTS by default).

**`backup*.sql` history finding (read-only `git show`):** `backup.sql` is an empty
0-byte blob. `backup_pre_phase8.sql` (`ea59f01`, ~483 KB) is a real `pg_dump` with **3
dev `users` rows** (two `@example.com` + the founder's own `@gmail.com`) + bcrypt hashes,
**2 `payments` rows** (provider IDs only, no card data), **1 `oauth_accounts` row**, dated
May 2026. Both are **already absent from the current `main` tree**. Classification:
**dev-stage data, not third-party production users** (signups not open) → not a prod-data
exposure; purge is *recommended hygiene*, not a blocker, and is a founder-gated history
rewrite (protected branch, `enforce_admins`). `.gitignore` already carries `backup*.sql`.

### Completed work (code)
- **`backend/app/core/security_headers.py`** (new) — pure-ASGI `SecurityHeadersMiddleware`
  mirroring `TraceIDMiddleware`. Appends `Strict-Transport-Security: max-age=86400;
  includeSubDomains` + `X-Content-Type-Options: nosniff`, **production-only**, never
  overwriting a header an endpoint set. Short 1-day max-age + `includeSubDomains` + **no
  preload** per the plan's "start with a short max-age" (founder-confirmed scope).
- **`backend/app/main.py`** — wired the middleware (outermost, after `TraceIDMiddleware`).
- **`frontend/next.config.ts`** — added async `headers()` emitting the same HSTS + nosniff
  on `/:path*`, inside the existing `withSentryConfig` wrapper.
- **No prod-code change to the guard** — re-confirmed via its existing test suite.

### Files changed
- `backend/app/core/security_headers.py` — new.
- `backend/app/main.py` — wire the middleware.
- `frontend/next.config.ts` — add `headers()`.
- `backend/tests/unit/platform/test_security_headers.py` — new (4 tests).
- `docs/G7_SECURITY_RUNBOOK.md` — new (git-ignored dir → `git add -f`).
- `docs/AGENT_PROGRESS.md` — this log.

### Verification performed
- `uv run python -m pytest tests/unit/platform/test_security_headers.py
  test_config_prod_guard.py` → **15 passed** (per memory: `python -m pytest`, and
  `SENTRY_DSN=""` to avoid the local-`.env` Sentry flush — same artifact noted in Session 6).
- `uv run ruff check` (3 changed files) → **All checks passed**;
  `uv run python -m mypy app/core/security_headers.py app/main.py` → **Success**.
- The middleware test exercises the **real ASGI stack** via `TestClient`: headers present
  in prod, absent in dev, present on an `HTTPException` (403) response, and an
  endpoint-set HSTS value is not clobbered.
- **`npm run build`** (frontend) → **success** with the new `headers()` config.
- Live secret-id format confirmed from Terraform (`lingosai/production/<KEY>`) so the
  rotation runbook's `put-secret-value` example is accurate.

### Findings
- Starlette nests `ServerErrorMiddleware` **outside** any `add_middleware` middleware, so a
  truly *unhandled* 500 (generated by that outermost layer) does **not** pass through our
  `send_wrapper` and won't carry HSTS. `HTTPException` responses (handled by the inner
  `ExceptionMiddleware`) **do**. Documented in the middleware + test; acceptable — HSTS on
  normal + handled-error responses is what matters, and prod 500s are rare + Sentry-tracked.
- HSTS needs **no** task-def/secret change — backend ships via CD on merge, frontend via the
  Vercel deploy of the same commit.

### Decisions
- **Short max-age (86400) + includeSubDomains + no preload** (founder-confirmed) — safe ramp;
  bump to 1 year / preload later in a one-line PR once HSTS is proven clean.
- **Production-only headers** — HSTS is meaningless/harmful without TLS; matches the
  guard's production-only posture and keeps local http dev untouched.
- **`backup*.sql` purge = founder-gated, recommended-not-required** — dev-only data, already
  out of the tree; the destructive history rewrite on a protected branch is a deliberate
  founder action. Agent documented the finding + exact `git filter-repo` procedure.
- **No prod mutation by the agent** — no deploy, no `put-secret-value`, no history rewrite.

### Risks
- The PR proves the *mechanism* (headers in the ASGI stack + a valid Vercel build), not
  that prod is serving them — that needs the deploy + the two `curl -sI` checks (runbook §1).
- `includeSubDomains` will force HTTPS on **all** `lingosai.com` subdomains once live; api +
  www are HTTPS, but any future http-only subdomain would break. Short max-age limits blast
  radius; founder confirmed all current subdomains are HTTPS.
- Provider keys remain dev-seeded until the founder rotates them (runbook §4) — until then,
  a leaked dev key is also a prod key.

### Founder actions required (to finish G7 — see `docs/G7_SECURITY_RUNBOOK.md`)
1. **Review + merge** PR `feat/g7-security-closeout` (backend HSTS ships via CD; frontend via Vercel).
2. Confirm HSTS live: `curl -sI https://api.lingosai.com/health` and `.../www.lingosai.com`
   both show `strict-transport-security` (runbook §1).
3. Rotate provider keys off dev-seeded values → Secrets Manager → `--force-new-deployment`
   (runbook §4); leave `JWT_SECRET`/`OTP_HASHING_SECRET`.
4. Record the `backup*.sql` decision (purge via runbook §3, or formally accept dev-only).

### Next recommended gap
**None remaining in the gap register** — G1–G8 are all code-complete. Once the founder
finishes the outstanding runbook steps across G4–G8 (live feature-parity proofs, email
external-inbox test, Sentry/alarm live capture, rollback/restore drill, HSTS confirm + key
rotation), **Appendix C is fully green → launch**. The agent's role on the plan is done
pending those founder-gated live proofs.
