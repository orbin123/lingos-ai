...
export function TaskWidgetRenderer({
  ...
  if (task.widget === "listen_tone") {
    return <ListenToneTaskWidget task={task} previewState={previewState} />;
  }
  ...
  if (task.widget === "read_context_mcq") {
    return <ReadContextMcqTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "read_tone_id") {
    return <ReadToneIdTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "read_structure") {
    return <ReadStructureTaskWidget task={task} previewState={previewState} />;
  }
  ...
  if (task.widget === "read_word_match") {
    return <ReadWordMatchTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "speak_pic_desc") {
    return <SpeakPicDescTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "write_paragraph") {
    return <WriteParagraphTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "speak_roleplay") {
    return <SpeakRoleplayTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "speak_interview") {
    return <SpeakInterviewTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "read_tfng") {
    return <ReadTfngTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "listen_shadow") {
    return <ListenShadowTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "write_email") {
    return <WriteEmailTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "speak_smalltalk") {
    return <SpeakSmalltalkTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "listen_retell") {
    return <ListenRetellTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "write_paraphrase") {
    return <WriteParaphraseTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "speak_present") {
    return <SpeakPresentTaskWidget task={task} previewState={previewState} />;
  }
  if (task.widget === "write_bullets_to_para") {
    return <WriteBulletsToParaTaskWidget task={task} previewState={previewState} />;
  }
