...
import type { SpeakSmalltalkTask } from "../source";
import {
  ResultBanner,
  RuleCallout,
  StatusDot,
...
export function SpeakSmalltalkTaskWidget({
  task,
  previewState,
}: {
  task: SpeakSmalltalkTask;
    ...
    <TaskWidgetFrame task={task} icon={<MessageSquare size={18} strokeWidth={2.5} />}>
      ...
      {!isDefault && (
        <ResultBanner
          ...
          label={`${correctCount} of 2 responses correct`}
        />
      )}
      ...
      >
        <div style={{ fontSize: 11, fontWeight: 800, color: "oklch(45% 0.07 240)", textTransform: "uppercase", letterSpacing: "0.05em", borderBottom: "1px dashed oklch(85% 0.03 240)", paddingBottom: 6 }}>
          Smalltalk Dialogue Flow
        </div>

        {task.dialogueContext.map((bubble, index) => {
          const isPartner = bubble.speaker === "partner";
          
          // Partner bubbles: index 0 and index 2
          // Learner turns: index 1 and index 3
          const learnerTurnIndex = index === 1 ? 0 : index === 3 ? 1 : -1;
